...
async function obtenerDatosAVGO() {
    try {
        const response = await fetch("https://financialmodelingprep.com/stable/historical-price-eod/light?symbol=AVGO&apikey=yUFAOz2TSOXSJNIqKMnivVdvuS0NspC8");
...
function calcularCambioAVGO(precioActual, precioAnterior) {
...
function crearCardAVGO(datos) {
    ...
    logo.src = "/assets/img/broadcom.png";  // logo
    ...
    document.getElementById("cardContainer7").appendChild(card);
}

// Ejecuta la obtención de datos y creación de la card
(async () => {
    const datos = await obtenerDatosAVGO();
