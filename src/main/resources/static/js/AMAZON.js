...
async function obtenerDatosAMZN() {
    try {
        const response = await fetch("https://financialmodelingprep.com/stable/historical-price-eod/light?symbol=AMZN&apikey=yUFAOz2TSOXSJNIqKMnivVdvuS0NspC8");
...
function calcularCambioAMZN(precioActual, precioAnterior) {
...
function crearCardAMZN(datos) {
    ...
    logo.src = "/assets/img/amazon.jpg";  // logo
...
(async () => {
    const datos = await obtenerDatosAMZN();
